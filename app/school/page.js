...
export default async function School({ searchParams }) {
  const url =
    "https://www.eporner.com/api/v2/video/search/?query=school&per_page=500&page=1&thumbsize=big&order=latest&gay=0&lq=1&format=json";
...
try {
  ...
  return (
    <>
      ...
      <h3 className="category-h3">School Category</h3>
      {paginatedVideos.length > 0 ? (
        <div className="container">
          ...
          <div className="pagination">
            {/* Previous Button */}
            {currentPage > 1 && (
              <a href={`/school/?page=${currentPage - 1}`} className="pagination-link">
                Previous
              </a>
            )}
