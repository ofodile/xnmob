...
export default async function Hardcore({ searchParams }) {
  const url =
    "https://www.eporner.com/api/v2/video/search/?query=hardcoreper_page=500&page=1&thumbsize=big&order=latest&gay=0&lq=1&format=json";
      ...
      {paginatedVideos.length > 0 ? (
        <div className="container">
          ...
          <div className="pagination">
            {/* Previous Button */}
            {currentPage > 1 && (
              <a href={`/hardcore/?page=${currentPage - 1}`} className="pagination-link">
                Previous
              </a>
            )}
